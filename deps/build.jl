    ...
    if !haskey(ENV, "R_HOME") && isdir(DepFile.Rhome) && validate_libR(DepFile.libR)
        ...
        elseif Rhome == "_"
            ...
            @static if Sys.iswindows()
                ...
                if isempty(Rhome)
                    try
                        Rhome = WinReg.querykey(WinReg.HKEY_CURRENT_USER,
                                                "Software\\R-Core\\R", "InstallPath")
                    catch
                    end
                end
        ...
        if isempty(Rhome)
            ...
            open(depfile, "w") do f
                ...
                return println(f, "const conda_provided_r = nothing")
            end
        else
            ...
            if DepFile.Rhome != Rhome || DepFile.libR != libR
                open(depfile, "w") do f
                    ...
                    return println(f, "const conda_provided_r = $(conda_provided_r)")
                end
            end
        end
    end
