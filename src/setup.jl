...
function initEmbeddedR()
    ...
    @static if Sys.iswindows()
        ...
        ccall((:R_set_command_line_arguments, libR), Cint, (Cint, Ptr{Ptr{Cchar}}),
              length(argv), argv)
        ...
        try
            unsafe_store!(cglobal((:EmitEmbeddedUTF8, RCall.libR), Cint), 1)
        catch
        end
    end
    ...
    @static if Sys.isunix()
        ...
        unsafe_store!(cglobal((:ptr_R_ReadConsole, libR), Ptr{Cvoid}),
                      @cfunction(read_console, Cint, (Cstring, Ptr{UInt8}, Cint, Cint)))
        unsafe_store!(cglobal((:ptr_R_WriteConsole, libR), Ptr{Cvoid}), C_NULL)
        unsafe_store!(cglobal((:ptr_R_WriteConsoleEx, libR), Ptr{Cvoid}),
                      @cfunction(write_console_ex, Nothing, (Ptr{UInt8}, Cint, Cint)))
        unsafe_store!(cglobal((:R_Consolefile, libR), Ptr{Cvoid}), C_NULL)
        unsafe_store!(cglobal((:R_Outputfile, libR), Ptr{Cvoid}), C_NULL)
        unsafe_store!(cglobal((:R_PolledEvents, libR), Ptr{Cvoid}),
                      @cfunction(polled_events, Nothing, ()))
    end
...
function __init__()
    ...
    isempty(Rhome) && error("No R installation was detected at RCall installation time. " *
    ...
    # # IJulia hooks
    return isdefined(Main, :IJulia) && Main.IJulia.inited && ijulia_init()
end
