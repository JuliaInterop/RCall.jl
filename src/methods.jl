...
for sym in (:isArray, :isComplex, :isEnvironment, :isExpression, :isFactor,
    ...
    @eval begin
        $sym(s::Ptr{S}) where {S<:Sxp} = ccall(($(string("Rf_", sym)), libR), Bool,
                                               (Ptr{SxpPtrInfo},), s)
...
for (S, J) in ((:LglSxp, "LOGICAL"), (:IntSxp, "INTEGER"),
    ...
    @eval dataptr(s::Ptr{$S}) = convert(Ptr{eltype($S)},
                                        ccall(($J, libR), Ptr{Cvoid}, (Ptr{UnknownSxp},),
                                              s))
end
...
function setindex!(r::RObject{S}, value, keys...) where {S<:VectorSxp}
    return setindex!(sexp(r), value, keys...)
end
function setindex!(r::RObject{S}, ::Missing, keys...) where {S<:VectorSxp}
    return setindex!(sexp(r), naeltype(S), keys...)
end
...
Set element of a VectorSxp by a label.
"""
function setindex!(s::Ptr{S}, value::Ptr{T},
                   label::AbstractString) where {S<:VectorSxp,T<:Sxp}
    ls = getnames(s)
    for (i, l) in enumerate(ls)
        if rcopy(String, l) == label
            s[i] = value
...
function setindex!(s::Ptr{S}, value, label::AbstractString) where {S<:VectorSxp}
    return setindex!(s, sexp(value), label)
end
function setindex!(s::Ptr{S}, value, label::Symbol) where {S<:VectorSxp}
    return setindex!(s, value, string(label))
end

# VectorAtomicSxp
getindex(s::Ptr{S}, I::Integer) where {S<:VectorAtomicSxp} = getindex(unsafe_vec(s), I)
function getindex(s::Ptr{S}, I::Integer...) where {S<:VectorAtomicSxp}
    return getindex(unsafe_array(s), I...)
end
function getindex(s::Ptr{S}, I::AbstractVector) where {S<:VectorAtomicSxp}
    return getindex(unsafe_vec(s), I)
end
...
function Base.checkbounds(::Type{Bool}, r::RObject{S}, i::Integer) where {S<:VectorListSxp}
    return 1 <= i <= length(r)
end
function Base.checkbounds(r::RObject{S}, i) where {S<:VectorListSxp}
...
function getindex(s::Ptr{StrSxp}, key::Integer)
    return c = ccall((:STRING_ELT, libR), Ptr{CharSxp}, (Ptr{StrSxp}, Cint), s, key - 1)
end
...
function setindex!(s::Ptr{S}, value::Ptr{T},
                   key::Integer) where {S<:Union{VecSxp,ExprSxp},T<:Sxp}
    1 <= key <= length(s) || throw(BoundsError())
    return ccall((:SET_VECTOR_ELT, libR), Ptr{T},
...
function cdr(s::Ptr{S}) where {S<:PairListSxp}
    return sexp(ccall((:CDR, libR), Ptr{UnknownSxp}, (Ptr{S},), s))
end
function car(s::Ptr{S}) where {S<:PairListSxp}
    return sexp(ccall((:CAR, libR), Ptr{UnknownSxp}, (Ptr{S},), s))
end
function tag(s::Ptr{S}) where {S<:PairListSxp}
    return sexp(ccall((:TAG, libR), Ptr{UnknownSxp}, (Ptr{S},), s))
end
...
@inline function iterate(x::Pairs{K,V,I,Ptr{S}}, state) where {K,V,I,S<:PairListSxp}
    state == sexp(Const.NilValue) && return nothing
    return (tag(state), car(state)), cdr(state)
end

IteratorSize(x::Pairs{K,V,I,RObject{S}}) where {K,V,I,S<:PairListSxp} = Base.SizeUnknown()
function IteratorEltype(x::Pairs{K,V,I,RObject{S}}) where {K,V,I,S<:PairListSxp}
    return Base.EltypeUnknown()
end
@inline iterate(x::Pairs{K,V,I,RObject{S}}) where {K,V,I,S<:PairListSxp} = iterate(x,
                                                                                   values(x).p)
@inline function iterate(x::Pairs{K,V,I,RObject{S}}, state) where {K,V,I,S<:PairListSxp}
    state == sexp(Const.NilValue) && return nothing
    return (RObject(tag(state)), RObject(car(state))), cdr(state)
end
...
"""
function setindex!(s::Ptr{S}, value::Ptr{T},
                   label::AbstractString) where {S<:PairListSxp,T<:Sxp}
    ls = getnames(s)
    for (i, l) in enumerate(ls)
        if rcopy(String, l) == label
            s[i] = value
...
function setindex!(s::Ptr{S}, value, label::AbstractString) where {S<:PairListSxp}
    return setindex!(s, sexp(value), label)
end
function setindex!(s::Ptr{S}, value, label::Symbol) where {S<:PairListSxp}
    return setindex!(s, value, string(label))
end
...
function getindex(s::Ptr{S4Sxp}, sym::Ptr{SymSxp})
    if ccall((:R_has_slot, libR), Int, (Ptr{S4Sxp}, Ptr{SymSxp}), s, sym) == 1
        return sexp(ccall((:R_do_slot, libR), Ptr{UnknownSxp}, (Ptr{S4Sxp}, Ptr{SymSxp}), s,
                          sym))
    else
        throw(BoundsError())
    end
...
function setindex!(s::Ptr{S4Sxp}, value::Ptr{T}, sym::Ptr{SymSxp}) where {T<:Sxp}
    protect(value)
    try
        t = rcall_p(findNamespace("methods")[:checkSlotAssignment], s, rcopy(String, sym),
                    value)
        sexp(ccall((:R_do_slot_assign, libR), Ptr{UnknownSxp},
                   (Ptr{S4Sxp}, Ptr{SymSxp}, Ptr{T}), s, sym, t))
    finally
        unprotect(1)
    end
end
...
function getattrib(s::Ptr{S}, sym::Ptr{SymSxp}) where {S<:Sxp}
    return sexp(ccall((:Rf_getAttrib, libR), Ptr{UnknownSxp}, (Ptr{S}, Ptr{SymSxp}), s,
                      sym))
end
...
function getclass(s::Ptr{S}, singleString::Bool=false) where {S<:Sxp}
    return ccall((:R_data_class, libR), Ptr{StrSxp}, (Ptr{S}, Cint), s, singleString)
end
...
function allocArray(::Type{S}, n::Integer) where {S<:Sxp}
    return ccall((:Rf_allocVector, libR), Ptr{S}, (Cint, Cptrdiff_t), sexpnum(S), n)
end

function allocArray(::Type{S}, n1::Integer, n2::Integer) where {S<:Sxp}
    return ccall((:Rf_allocMatrix, libR), Ptr{S}, (Cint, Cint, Cint), sexpnum(S), n1, n2)
end

function allocArray(::Type{S}, n1::Integer, n2::Integer, n3::Integer) where {S<:Sxp}
    return ccall((:Rf_alloc3DArray, libR), Ptr{S}, (Cint, Cint, Cint, Cint), sexpnum(S), n1,
                 n2, n3)
end

function allocArray(::Type{S}, dims::Integer...) where {S<:Sxp}
    sdims = sexp(RClass{:integer}, [dims...])
    return ccall((:Rf_allocArray, libR), Ptr{S}, (Cint, Ptr{IntSxp}), sexpnum(S), sdims)
end
...
function findVarInFrame(e::Ptr{EnvSxp}, s::Ptr{SymSxp})
    return ccall((:Rf_findVarInFrame, libR), Ptr{UnknownSxp}, (Ptr{EnvSxp}, Ptr{SymSxp}), e,
                 s)
end
...
function setindex!(e::Ptr{EnvSxp}, v::Ptr{S}, s::Ptr{StrSxp}) where {S<:Sxp}
    ...
    return rcall_p(Const.BaseNamespace["assign"], s, v; envir=e)
end
...
function newEnvironment(env::Ptr{EnvSxp})
    return ccall((:Rf_NewEnvironment, libR), Ptr{EnvSxp},
                 (Ptr{NilSxp}, Ptr{NilSxp}, Ptr{EnvSxp}), sexp(Const.NilValue),
                 sexp(Const.Const.NilValue), env)
end
...
function findNamespace(str::String)
    return ccall((:R_FindNamespace, libR), Ptr{EnvSxp}, (Ptr{StrSxp},), sexp(str))
end
