...
using ..RCall: libR, RObject, NilSxp, SymSxp, EnvSxp, SxpHead, StrSxp, CharSxp
...
macro load_const(s)
    return :($s.p = unsafe_load(cglobal(($(string(:R_, s)), libR), typeof($s.p))))
end

macro load_const_embedded(s)
    return :($s.p = unsafe_load(cglobal($(string(:R_, s)), typeof($s.p))))
end
